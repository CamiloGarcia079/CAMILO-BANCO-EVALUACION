...
function setupForms() {
    ...
    document.getElementById('topupForm')?.addEventListener('submit', (e) => {
        ...
        const operator = document.getElementById('topupOperator').value.trim();
        const phone = document.getElementById('topupPhone').value.trim();
        const amount = parseFloat(document.getElementById('topupAmount').value);

        const recargaExitosa = realizarRecargaTelefonica(operator, phone, amount);
...
function realizarRecargaTelefonica(operator, phone, amount) {
    const operadoresPermitidos = ['MovilNet', 'Amber Phone', 'Dynaphone', 'SkyPhone'];
    const telefonoNormalizado = phone.replace(/\D/g, '');

    if (!operadoresPermitidos.includes(operator)) {
        showToast('Selecciona un operador válido', 'error');
        return false;
    }

    if (!/^\d{10}$/.test(telefonoNormalizado)) {
        showToast('Ingresa un número telefónico válido de 10 dígitos', 'error');
        return false;
    }

    if (!Number.isFinite(amount) || amount <= 0) {
        showToast('Ingresa un valor de recarga mayor a 0', 'error');
        return false;
    }

    if (amount > currentUser.balance) {
        showToast('Saldo insuficiente para realizar la recarga', 'error');
        return false;
    }

    const descripcion = `Recarga ${operator} al número ${telefonoNormalizado}`;
    const tx = registerTransaction('Retiro', descripcion, amount);

    showToast('Recarga telefónica realizada con éxito', 'success');
    showPrintModalForTransaction(tx);
